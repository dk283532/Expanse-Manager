...
class FirebaseRegisterFragment : Fragment() {
    ...
    private fun UpdateUI(currentUser: FirebaseUser?) {
        ...
        if (currentUser != null) {
            findNavController().navigate(FirebaseRegisterFragmentDirections.actionFirebaseRegisterFragmentToTransactionListFragment2())
        }
    }
            ...
            if (validateInput(email, pass)) {
                ...
                auth.createUserWithEmailAndPassword(email, pass).addOnCompleteListener { task ->
                    ...
                    if (task.isSuccessful) {
                        Toast.makeText(requireContext(),"Registration is Successfull",Toast.LENGTH_SHORT).show()
                        findNavController().navigate(FirebaseRegisterFragmentDirections.actionFirebaseRegisterFragmentToLoginFragment())
                    } else {
